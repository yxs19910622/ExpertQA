...
var colModel = 
    [
     	 ...
     	 */
     	 {header:"点击区域" , name:"category" , rowspan:2 , width:50,align : "center",sort:'serverSide'}, 
     	 /**
     	 *   "联系方式"是没有实际业务意义的，它只是作为"地址"、"联系方式"、"公司主页"这三列的统称，所以这里一定不要加上 "name"属性，切记！
     	 *   另外，由于"联系方式"实际上占了三列，所以要加上 colspan:3，不然会得不到正确的视图。
     	 */
     	 {header:"用户在待续费期间" , colspan:2},
     	 {header:"付费用户在待续费期" , colspan:2},
     	 {header:"用户付费期间" , colspan:2},
     	 {header:"待续费用户在付费期" , colspan:2},
     	 {header:"付费和待续费用户在付费之前" , colspan:2},
     	 {header:"未付费用户" , colspan:2},
     	 {header:"游客" , colspan:2},
     	 {header:"注册客户在注册之前" , colspan:2}],
     	...
     	[{header:"总人数" , name:"headcount1" , width:50,align : "center",sort:'serverSide'},
     	 ...
     	 {header:"总人数" , name:"headcount5" , width:100,align : "center",sort:'serverSide'},
...
$('#payGrid').omGrid({
    ...
    autoFit : false,
    colModel : colModel,
    dataSource : 'setHome!ajaxStat_2.action',
...
function snake_1(){
	...
	for(var i=0;i<obj.length;i++)
	{
		...
		if(obj[i].className == '')//找出div标记中class= 的那个标记
		{
			var getObj = obj[i];
		...
		}else if(obj[i].className == 'innerCol '){//找出div标记中class=innerCol 的那个标记
			var getObj1 = obj[i];
			value1 = getObj1.innerHTML;//获得他的innerHTML
			if(value1=='应用'){
				getObj1.onmouseover=function(){
					show("divs1");
				};
				getObj1.onmouseout=function(){
					hide("divs1");
				};
			}else if(value1=='账户'){
				getObj1.onmouseover=function(){
...
<jsp:param value="PayHistory" name="currOpt"/>
    ...
    <div class="lesson_tit">区域点击统计</div>
    <tr> 
...
<button onclick="snake_2()">查看用户类型定义</button>
	...
	<div id="divs1" style="color:red;position:absolute;display:none;border:1px solid silver;background:silver;">软件登录、软件退出、一键更新</div>
	<div id="divs2" style="color:red;position:absolute;display:none;border:1px solid silver;background:silver;">账号登录、注册、账户安全退出、密码重置、我要付款、个人中心（点击账户名称）</div>
	<div id="divs3" style="color:red;position:absolute;display:none;border:1px solid silver;background:silver;">专家在线、联系我们、向专家提问、用户手册、关于我们</div>
	<div id="divs4" style="color:red;position:absolute;display:none;border:1px solid silver;background:silver;">劳动法所有工具，含计算器、Word模板、Excel模板等</div>
	<div id="divs5" style="color:red;position:absolute;display:none;border:1px solid silver;background:silver;">点击的目录栏里任意文档（含下载）</div>
	<div id="divs6" style="color:red;position:absolute;display:none;border:1px solid silver;background:silver;">HR新闻、企业新闻（标题）</div>
<script type="text/javascript">
function snake(){
	...
	$('#payGrid').omGrid({
	    ...
	    autoFit : false,
	    colModel : colModel,
	    dataSource : 'setHome!ajaxStat_2.action?sd='+sd+'&ed='+ed+'&zt='+zt
	});
}
function snake_2(){
...
function showDIV(data,name){
	...
	$('#grid').omGrid({
	    ...
	    dataSource : 'setHome!ajaxStat_2.action?type='+data+'&name='+name+'&sys='+s+'&zt='+zt,
